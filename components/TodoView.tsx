...
const PriorityBadge: React.FC<{ priority: Priority; onClick: () => void }> = ({ priority, onClick }) => {
  ...
  return (
    <button 
      ...
      className={`px-2.5 py-0.5 rounded-full text-[10px] font-grotesk font-bold uppercase tracking-widest border transition active:scale-[0.95] ${getColors()}`}
    >
      {priority}
    </button>
  );
...
const TaskItem: React.FC<{ 
  ...
  return (
    <motion.div 
      ...
      className={`group bg-surface rounded-2xl p-5 mb-4 border border-surface-highlight transition-shadow duration-200 ${task.completed ? 'opacity-60' : 'shadow-sm hover:shadow-md'}`}
    >
      <div className="flex items-start gap-2 sm:gap-4">
        ...
        >
          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${task.completed ? 'bg-accent border-accent scale-105' : 'border-secondary hover:border-accent'}`}>
            {task.completed && <Check className="w-3.5 h-3.5 text-accent-fg" />}
          </div>
        </button>
        <div className="flex-grow pt-2">
          <span className={`block text-lg transition-colors duration-200 ${task.completed ? 'line-through text-secondary' : 'text-primary'}`}>{task.text}</span>
        ...
        <div className="flex flex-row sm:flex-col gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity pt-1">
          <button 
            ...
            className="w-11 h-11 flex items-center justify-center text-secondary hover:text-accent rounded-lg transition-all active:scale-[0.92]"
          >
             {loadingSubtasks ? <Bot className="w-5 h-5 animate-pulse text-accent" /> : <Bot className="w-5 h-5"/>}
          </button>
          <button onClick={handleDelete} title="Delete Task" className="w-11 h-11 flex items-center justify-center text-secondary hover:text-red-600 rounded-lg transition-all active:scale-[0.92]"><Trash2 className="w-5 h-5" /></button>
        </div>
      </div>
      {(isExpanded || loadingSubtasks) && (
        <motion.div 
          ...
          transition={iosSpring}
          className="mt-4 pl-4 sm:pl-12 space-y-4 overflow-hidden"
        >
           {loadingSubtasks && (
             <div className="py-2 space-y-3">
               <div className="flex items-center gap-3">
                  <div className="w-6 h-6 rounded border border-surface-highlight/50 bg-surface-highlight/40 animate-pulse"></div>
                  <div className="h-4 bg-surface-highlight/40 rounded w-3/4 animate-pulse"></div>
               </div>
               <div className="flex items-center gap-3">
                  <div className="w-6 h-6 rounded border border-surface-highlight/50 bg-surface-highlight/40 animate-pulse"></div>
                  <div className="h-4 bg-surface-highlight/40 rounded w-1/2 animate-pulse"></div>
               </div>
  ...
  return (
    <div className="pb-32 px-4 sm:px-6 max-w-3xl mx-auto w-full">
      <div className="mb-10 mt-4 flex items-center gap-4">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-display font-bold text-primary tracking-tight">Tasks</h2>
      </div>
      <div className="relative mb-10 flex items-center">
        ...
        {inputText.trim().length > 0 && (
          <button
            onClick={() => { triggerHaptic(10); onAddTask(inputText); setInputText(''); }}
            title="Add task"
        ...
        <div className="space-y-1">
          {activeTasks.length === 0 && completedTasks.length > 0 && (
            <motion.div 
              ...
              transition={iosSpring}
              className="mb-8 p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300 flex items-center gap-4"
            >
          ...
          <AnimatePresence mode="popLayout">
            {activeTasks.map(task => (
              <TaskItem key={task.id} task={task} onToggle={() => onToggleTask(task.id)} onDelete={() => onDeleteTask(task.id)} onUpdate={onUpdateTask} completionAnim={completionAnim} deleteAnim={deleteAnim} selectedModel={selectedModel} />
            ))}
          </AnimatePresence>
