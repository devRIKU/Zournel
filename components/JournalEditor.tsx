...
import { 
  ...
  List, Code, Undo, Redo, Terminal, BrainCircuit, Type, FileText
} from 'lucide-react';
...
export const JournalEditor: React.FC<JournalEditorProps> = ({ 
  ...
  const [activeStates, setActiveStates] = useState<ActiveStates>({
    bold: false, italic: false, code: false, codeBlock: false, bulletList: false
  });
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-bg flex flex-col animate-fade-in overflow-hidden" role="dialog" aria-modal="true">
      {/* Cover Section */}
      <div className="relative h-[20vh] sm:h-[25vh] shrink-0 w-full overflow-hidden bg-surface-highlight">
        {!imageLoaded && (
        ...
        <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center z-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleManualSave} className="px-5 py-2.5 bg-accent text-accent-fg rounded-xl text-xs font-bold shadow-xl hover:bg-accent/90 active:scale-95 transition-all flex items-center gap-2 border border-white/10 outline-none">
      ...
      {/* REFINED TOOLBAR */}
      <div className="shrink-0 bg-surface border-b border-surface-highlight shadow-sm z-30 px-2 sm:px-6 py-2 overflow-x-auto no-scrollbar">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="hidden md:flex items-center gap-3 text-secondary font-mono text-[10px] uppercase tracking-widest bg-surface-highlight/50 px-4 py-2 rounded-full border border-surface-highlight">
             <div className="flex items-center gap-2">
                <FileText className="w-3 h-3 text-accent" />
                <span>{wordCount} words</span>
             </div>
             <div className="w-px h-3 bg-secondary/30"></div>
             <span>Drafting</span>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative" ref={aiMenuRef}>
              <button onClick={() => setShowAiMenu(!showAiMenu)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${showAiMenu ? 'bg-accent text-accent-fg shadow-lg' : 'bg-accent/5 text-accent hover:bg-accent/10'}`}>
                <Sparkles className={`w-3.5 h-3.5 ${isProcessing ? 'animate-pulse' : ''}`} />
                <span className="hidden sm:inline">AI Assistant</span>
              </button>
              {showAiMenu && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-surface border border-surface-highlight rounded-2xl shadow-2xl p-2 flex flex-col z-[110] animate-scale-in">
                  <div className="px-3 py-2 text-[9px] font-bold text-secondary uppercase tracking-tighter opacity-50 border-b border-surface-highlight mb-1">Editor Tools</div>
                  <button onClick={() => handleAIEdit('IMPROVE')} className="flex items-center gap-3 px-3 py-3 hover:bg-accent/5 text-primary text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all"><Wand2 className="w-4 h-4 text-accent"/> Polish Flow</button>
                  <button onClick={() => handleAIEdit('REPHRASE')} className="flex items-center gap-3 px-3 py-3 hover:bg-accent/5 text-primary text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all"><Type className="w-4 h-4 text-accent"/> Elegant Style</button>
                  <div className="h-px bg-surface-highlight my-1"></div>
                  <div className="px-3 py-2 text-[9px] font-bold text-secondary uppercase tracking-tighter opacity-50 border-b border-surface-highlight mb-1">Insights</div>
                  <button onClick={() => setShowAiMenu(false)} className="flex items-center gap-3 px-3 py-3 hover:bg-accent/5 text-primary text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all"><BrainCircuit className="w-4 h-4 text-accent"/> Extract Tasks</button>
                </div>
              )}
            </div>
          </div>
      ...
      {showImagePrompt && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-6" role="dialog" aria-modal="true">
          <div className="bg-surface rounded-[2rem] w-full max-w-md p-8 shadow-2xl border border-white/5 animate-scale-in">
            ...
            <div className="space-y-6">
              <button onClick={() => localImageInputRef.current?.click()} className="w-full py-8 bg-surface-highlight hover:bg-accent hover:text-accent-fg border-2 border-dashed border-accent/20 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all">
